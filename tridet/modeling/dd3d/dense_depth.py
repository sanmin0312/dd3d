...
class DD3DDenseDepthHead(nn.Module):
    def __init__(self, cfg, input_shape):
        ...
        self.dense_depth = nn.ModuleList([
            Conv2d(in_channels, 3, kernel_size=3, stride=1, padding=1, bias=(not cfg.DD3D.FCOS3D.USE_SCALE))
    ...
    def forward(self, x):
        ...
        for l, features in enumerate(x):
            ...
            dense_depth_lvl = self.dense_depth[l](features)
            if self.use_scale:
...
# @META_ARCH_REGISTRY.register()
# class DD3DVIDEODenseDepth_single(nn.Module):
#     def __init__(self, cfg, input_shape):
#         super().__init__()
#         self.in_features = cfg.DD3D.IN_FEATURES
#         self.feature_locations_offset = cfg.DD3D.FEATURE_LOCATIONS_OFFSET
...
#
#         dense_depth = self.fcos3d_head(x)[2]
#
#         inv_intrinsics = intrinsics.inverse() if intrinsics is not None else None
#
#         # Upsample.
#         dense_depth = aligned_bilinear(dense_depth, factor=self.in_strides[2], offset=self.feature_locations_offset).squeeze(1)
#
#         if self.scale_depth_by_focal_lengths:
#             assert inv_intrinsics is not None
#             pixel_size = torch.norm(torch.stack([inv_intrinsics[:, 0, 0], inv_intrinsics[:, 1, 1]], dim=-1), dim=-1)
#             scaled_pixel_size = (pixel_size * self.scale_depth_by_focal_lengths_factor).reshape(-1, 1, 1)
#             dense_depth = dense_depth / scaled_pixel_size
#
#         return dense_depth
