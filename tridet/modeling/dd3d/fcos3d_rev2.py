...
class FCOS3DHead(nn.Module):
    def __init__(self, cfg, input_shape, depth=False):
        ...
        if self.depth_on:
            self.dense_depth = nn.ModuleList([
                Conv2d(in_channels, 3, kernel_size=3, stride=1, padding=1, bias=(not self.use_scale))
                for _ in range(self.num_levels)
            ])

            self.depth_uncert = nn.ModuleList([
                Conv2d(in_channels, 2, kernel_size=3, stride=1, padding=1, bias=True)
                for _ in range(self.num_levels)
            ])
    ...
    def forward(self, x, inv_intrinsics=None):
        box3d_quat, box3d_ctr, box3d_depth, box3d_size, box3d_conf, dense_depth, ego_pose, depth_uncert_c, depth_uncert_d = [], [], [], [], [], [], [], [], []
        ...
        for l, features in enumerate(x):
            ...
            if self.depth_on:
                ...
                depth_uncert_c.append(depth_un[:, 0])
                depth_uncert_d.append(depth_un[:, 1])
        ...
        return box3d_quat, box3d_ctr, box3d_depth, box3d_size, box3d_conf, dense_depth, dense_depth_low, depth_uncert_c,  depth_uncert_d
...
class FCOS3DHead_prediction(nn.Module):
    ...
    def forward(self, x, inv_intrinsics=None):
        box3d_quat, box3d_ctr, box3d_depth, box3d_size, box3d_conf, dense_depth, ego_pose, depth_uncert_p = [], [], [], [], [], [], [], []
        if not self.depth_on:
            dense_depth = None
        ...
        for l, features in enumerate(x):
            ...
            if self.depth_on:
                dense_depth.append(dense_depth_lvl)
                depth_uncert_p.append(depth_un)
        ...
        return box3d_quat, box3d_ctr, box3d_depth, box3d_size, box3d_conf, dense_depth, dense_depth_low, ego_pose, depth_uncert_p
...
class FCOS3DLoss():
    def __init__(self, cfg, prediction=False):
        ...
        self.prediction_on = prediction
        if self.prediction_on:
            self.prediction_loss_weight = cfg.DD3D.FCOS3D.LOSS.PREDICTION_WEIGHT
    ...
    def __call__(
        ...
        if pos_inds.numel() == 0:
            ...
            if self.prediction_on:
                losses = {
                    "loss_box3d_quat_pred": box3d_quat.sum() * 0.,
                    "loss_box3d_proj_ctr_pred": box3d_ctr.sum() * 0.,
                    "loss_box3d_depth_pred": box3d_depth.sum() * 0.,
                    "loss_box3d_size_pred": box3d_size.sum() * 0.,
                    "loss_conf3d_pred": box3d_conf.sum() * 0.
                }
            return losses
        ...
        if self.prediction_on:
            losses_box3d = {'{}_pred'.format(k): self.prediction_loss_weight * self.box3d_loss_weight * v / loss_denom for k, v in losses_box3d.items()}
        else:
        ...
        if self.prediction_on:
            loss_conf3d = self.prediction_loss_weight * self.conf3d_loss_weight * (loss_conf3d * centerness_targets).sum() / loss_denom
            losses = {"loss_conf3d_prev": loss_conf3d, **losses_box3d}
