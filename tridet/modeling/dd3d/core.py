...
"PREDICTION4 + temporal depth uncertainty"
@META_ARCH_REGISTRY.register()
class DD3D_VIDEO_PREDICTION8(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        for f in self.in_features:
            ...
            query.append(self.query(feat_prev[:1]))
            pos_embed.append(self.positional_encoder(feat_prev[:1]))
        ...
        if self.training:
            ...
            if depth_un is not None:
                for lvl, (c, p, un) in enumerate(zip(dense_depth_low, dense_depth_low_prev, depth_un)):
                    loss_lvl = ((torch.abs(c-p) / (torch.exp(-un))) + un) / (np.sqrt(2) ** (lvl + 1))
