...
class P2D_pos(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        for f in self.in_features:
            ...
            pos_embedding = self.positional_embedding(feat_cur)

            # feat_cur = features[f].reshape(3, batch_size, features[f].size(1), features[f].size(2), -1)[0]
            # feat_prev = features[f].reshape(3, batch_size, features[f].size(1), features[f].size(2), -1)[1:]

            features_cur.append(feat_cur+pos_embedding)
            features_prev.append((feat_prev+pos_embedding).transpose(0,1))
...
class P2D_0915(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        # if True:
            ...
            #ego pose loss
            if gt_ego_pose_prev is not None and ego_pose is not None:
                for lvl, x in enumerate(ego_pose):
...
class P2D_0923(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.VIDEO_ON:
            self.pose_loss_prev = PoseLoss(cfg)
            self.temporal_head = DeformableTransformer(cfg, )
            self.attention_warping = DeformableTransformer_warping(cfg, )
    ...
    def forward(self, batched_inputs):
        ...
        # if True:
            ...
            if self.prediction_on:
                ...
                logits_prev, box2d_reg_prev, centerness_prev, _ = self.fcos2d_head_prediction(features_prev)
                if not self.only_box2d:
                    box3d_quat_prev, box3d_ctr_prev, box3d_depth_prev, box3d_size_prev, box3d_conf_prev, dense_depth_prev, ego_pose_prev = self.fcos3d_head_prediction(
                        features_prev, inv_intrinsics)
            ...
            if gt_ego_pose is not None and ego_pose is not None:
                for lvl, x in enumerate(ego_pose):
                    loss_lvl = self.pose_loss_prev(x, gt_ego_pose)["loss_ego_pose"]
                    loss_lvl = loss_lvl / (np.sqrt(2) ** (3-lvl)) *2  # Is sqrt(2) good?
                    losses.update({f"loss_ego_pose_lvl_{lvl}": loss_lvl})
...
@META_ARCH_REGISTRY.register()
class P2D_0923_woPred(nn.Module):
    #TODO Positional Encoding for DeformAtt
    #TODO Temporal Feature Aggregation for current and past
...
@META_ARCH_REGISTRY.register()
class P2D_FS2_woPred(nn.Module):
    #TODO Positional Encoding for DeformAtt
    #TODO Temporal Feature Aggregation for current and past
...
@META_ARCH_REGISTRY.register()
class P2D_FS2_DepthPred(nn.Module):
    # TODO Positional Encoding for DeformAtt
    # TODO Temporal Feature Aggregation for current and past
    ...
    def forward(self, batched_inputs):
        ...
        for f in self.in_features:
            pos_embedding = self.positional_embedding(features[f][:1])
            feat = features[f].reshape(3, batch_size, features[f].size(1), features[f].size(2),
                                       -1) + self.temporal_embedding[:, None, :, None, None] + pos_embedding.unsqueeze(
                dim=0)
        ...
        # past pose estimation
        ego_pose_prev, pose_prev_embed = self.pose_haed(
            [x.reshape(x.shape[0], -1, x.shape[-2], x.shape[-1]) for x in features_prev])
        ego_pose, _ = self.pose_haed([torch.cat((x1, x2[:, 0]), dim=1) for x1, x2 in zip(features_cur, features_prev)])

        # temporal aggregation
        features_prev = self.temporal_head(features_prev, masks=None,
                                           pos_embeds=None)  # features_prev = [B, T, C, H, W]

        features_prev = self.attention_warping(
            [torch.stack((feat_prev + embed[:, :, None, None], feat_prev), dim=1) for feat_prev, embed in
             zip(features_prev, pose_prev_embed)], masks=None, pos_embeds=None)
        # features = [torch.stack((f_c, f_p), dim=1) for f_p, f_c in zip(features_prev, features_cur)]
        # features = self.temporal_head(features, masks=None, pos_embeds=None)
        ...
        if self.training:
            ...
            if self.prediction_on:
                # prediction from previous images
                if not self.only_box2d:
                    _, _, _, _, _, dense_depth_prev, _ = self.fcos3d_head_prediction(
                        features_prev, inv_intrinsics)
