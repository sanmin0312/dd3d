...
class DenseDepthEval(nn.Module):
    @configurable
    def __init__(self, beta, min_depth=0., max_depth=100.):
        super().__init__()
        self.beta = beta
        self.min_depth = min_depth
        self.max_depth = max_depth
    ...
    def forward(self, depth_pred, depth_gt, masks=None):
        ...
        M = M == 0.
        loss = depth_pred[M] - depth_gt[M]

        return {"loss_dense_depth": loss}


class DenseDepthL1LossWithUncertainty(nn.Module):
    @configurable
    def __init__(self, beta, min_depth=0., max_depth=100., loss_weight=1.0):
        super().__init__()
        self.beta = beta
    ...
    def forward(self, depth_pred, depth_gt, uncertainty, masks=None):
        M = (depth_gt < self.min_depth).to(torch.float32) + (depth_gt > self.max_depth).to(torch.float32)
        if masks is not None:
            M += (1. - masks).to(torch.float32)
        ...
        pred = ((torch.abs(depth_pred[M] - depth_gt[M]) / (torch.exp(uncertainty) + 1e-7)) + uncertainty)

        loss = smooth_l1_loss(pred, torch.zeros_like(pred), beta=self.beta, reduction='mean')
...
def build_dense_depth_uncertainty_loss(cfg):
    if cfg.DD3D.FCOS3D.DEPTH_HEAD.LOSS_TYPE == "L1":
        return DenseDepthL1LossWithUncertainty(cfg)
    else:
        ValueError(f"Not supported depth loss: {cfg.DD3D.FCOS3D.DEPTH_HEAD.LOSS_TYPE}")