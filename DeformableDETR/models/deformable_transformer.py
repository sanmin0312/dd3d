...
class DeformableTransformer_warping(nn.Module):
    def __init__(self, cfg):
        ...
        encoder_layer = DeformableTransformerEncoderLayer_warpping(d_model, dim_feedforward,
                                                          dropout, activation,
    ...
    def forward(self, srcs, masks=None, pos_embeds=None):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            qs_flatten.append(q)
            kvs_flatten.append(kv)

            mask_flatten.append(mask)
        qs_flatten = torch.cat(qs_flatten, 1)
        kvs_flatten = torch.cat(kvs_flatten, 1)
        ...
        # encoder
        memory = self.encoder(qs_flatten, kvs_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten)
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, q, kv, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        # cross attention
        src2 = self.cross_attn(self.with_pos_embed(q, pos), reference_points, kv, spatial_shapes, level_start_index, padding_mask)
...
class DeformableTransformerEncoderLayer_warpping(nn.Module):
    def __init__(self,
                 d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4):
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, q, kv, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        output = q
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=q.device)
        for _, layer in enumerate(self.layers):
            output = layer(output, kv, pos, reference_points, spatial_shapes, level_start_index, padding_mask)
            # output = checkpoint.checkpoint(layer, output, kv, pos, reference_points, spatial_shapes, level_start_index, padding_mask)
